...
public class NoteManager
{
	...
	public static final String[] FULL_PROJECTION = { Note.ID, Note.TITLE, Note.FILE, Note.NOTE_CONTENT, Note.MODIFIED_DATE, Note.ENCRYPTED, Note.ENCRYPTED_ALGORITHM };
	public static final String[] LIST_PROJECTION = { Note.ID, Note.TITLE, Note.MODIFIED_DATE, Note.FILE,Note.ENCRYPTED, Note.ENCRYPTED_ALGORITHM };
	...
	public static Note getNote(Activity activity, Uri uri)
	{
		...
		if (!(cursor == null || cursor.getCount() == 0))
		{
			...
			String noteEncrypted = cursor.getString(cursor.getColumnIndexOrThrow(Note.ENCRYPTED));
			String noteEncryptedAlgorithm = cursor.getString(cursor.getColumnIndexOrThrow(Note.ENCRYPTED_ALGORITHM));
			String file = cursor.getString(cursor.getColumnIndexOrThrow(Note.FILE));

			note = new Note();
			note.setXmlContent(noteContent);
			note.setTitle(noteTitle);
			note.setEncrypted(noteEncrypted.trim().equalsIgnoreCase("true"));
			note.setEncryptedAlgorithm(noteEncryptedAlgorithm);
			note.setFileName(file);
		}
	...
	public static void putNote(Activity activity, Note note)
	{
		...
		Cursor managedCursor = cr.query(notes, EMPTY_PROJECTION, Note.GUID + "= ?", whereArgs, null);
		...
		// Notice that we store the date in UTC because sqlite doesn't handle
		// RFC3339 timezone information
		Log.v("blabla", note.getLastChangeDate() + "");
		...
		values.put(Note.ENCRYPTED_ALGORITHM, note.getEncryptedAlgorithm());
		...
		if (managedCursor.getCount() == 0)
		{
			...
			if (Tomdroid.LOGGING_ENABLED)
				Log.v(TAG, "Note inserted in content provider. ID: " + uri + " TITLE:" + note.getTitle() + " GUID:" + note.getGuid());
		} else
		{
			...
			if (Tomdroid.LOGGING_ENABLED)
				Log.v(TAG, "Note updated in content provider. TITLE:" + note.getTitle() + " GUID:" + note.getGuid());
		}
	}
	...
	// gets the titles of the notes present in the db, used in
	// ViewNote.buildLinkifyPattern()
	...
	// gets the ids of the notes present in the db, used in
	// SyncService.deleteNotes()
