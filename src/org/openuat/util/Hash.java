...
//#endif
	...
	public static byte[] normalSHA256(byte[] text, boolean useJSSE)
			throws InternalApplicationException {
		/*
		 * This double hashing with the first hash being prepended to the
		 * message is suggested by Practical Cryptography, p. 93 - it should
		 * solve the length extension attacks and thus the MD5 attacks. It is
		 * defined as SHAd256(m) = SHA256( SHA256(m) | m ).
		 */

		return SHA256(text, useJSSE);
	}
