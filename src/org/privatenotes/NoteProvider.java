...
public class NoteProvider extends ContentProvider {
	...
	private static final String DEFAULT_SORT_ORDER = Note.MODIFIED_DATE
			+ " DESC";
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			  db.execSQL("CREATE TABLE " + DB_TABLE_NOTES	 + " ("
                 ...
                 + Note.TAGS + " STRING,"
                 + Note.ENCRYPTED + " TEXT,"
                 + Note.ENCRYPTED_ALGORITHM + " TEXT,"
                 + Note.IS_SYNCED + " INTEGER"
                 + ");");
		}

		public void updateDB(Note n) {
			SQLiteDatabase db = getWritableDatabase();
			String guid = n.getGuid().toString();

			//noch mal kontrollieren
			String title= n.getTitle();
			String file= n.getFileName();
			
			String noteContent= n.getXmlContent();
			String modifiedDate= new Time().toString();
			String tags= n.getTags();
			boolean encrypted = n.isEncrypted();
			String encryptedAlgorithm=n.getEncryptedAlgorithm();
			int id = n.getDbId();
			int isSynced = n.isSynced() ? 1 : 0 ;
			db.execSQL("UPDATE "+DB_TABLE_NOTES+"SET " +
         		//variablen anlegen und von der klasse Note holen.
         		Note.GUID + " ="+guid+", "
                 + Note.TITLE + " ="+title+", "
                 + Note.FILE + " ="+file+", "
                 + Note.NOTE_CONTENT + " ="+noteContent+", "
                 + Note.MODIFIED_DATE + " ="+modifiedDate+", "
                 + Note.TAGS + " ="+tags+", "
                 + Note.ENCRYPTED + " ="+encrypted+", "
                 + Note.ENCRYPTED_ALGORITHM + " ="+encryptedAlgorithm+", "
                 + Note.IS_SYNCED + " ="+isSynced +
                 " WHERE "+ Note.ID + " ="+id);
		}
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case NOTE_TITLE:
			qb.setTables(DB_TABLE_NOTES);
			qb.setProjectionMap(notesProjectionMap);
			// TODO appendWhere + whereArgs instead (new String[] whereArgs =
			// uri.getLas..)?
	...
	// TODO the following method is probably never called and probably wouldn't
	// work
	@Override
	public Uri insert(Uri uri, ContentValues initialValues) {
		...
		// TODO either be identical to Tomboy's time format (if sortable) else
		// make sure that this is documented
		...
		if (values.containsKey(Note.ENCRYPTED) == false) {
			values.put(Note.ENCRYPTED, "false");
		}

		if (values.containsKey(Note.ENCRYPTED_ALGORITHM) == false) {
			values.put(Note.ENCRYPTED_ALGORITHM, "");
		}

		SQLiteDatabase db = dbHelper.getWritableDatabase();
		long rowId = db.insert(DB_TABLE_NOTES, Note.FILE, values); // not so
		// sure I
		// did the
		// right
		// thing
		// here
		if (rowId > 0) {
			Uri noteUri = ContentUris.withAppendedId(Tomdroid.CONTENT_URI,
					rowId);
	...
	public int delete(Uri uri, String where, String[] whereArgs) {
		...
		case NOTE_ID:
			...
			count = db.delete(DB_TABLE_NOTES,
					Note.ID
							+ "="
							+ noteId
	...
	public int update(Uri uri, ContentValues values, String where,
		...
		case NOTE_ID:
			String noteId = uri.getPathSegments().get(1);
			count = db.update(DB_TABLE_NOTES, values,
					Note.ID
							+ "="
							+ noteId
							+ (!TextUtils.isEmpty(where) ? " AND (" + where
	...
	static {
		...
		notesProjectionMap.put(Note.ENCRYPTED, Note.ENCRYPTED);
		notesProjectionMap.put(Note.ENCRYPTED_ALGORITHM,
				Note.ENCRYPTED_ALGORITHM);
