...
/**
 * shared Sd card sync server, works together with the SdCardSyncServiceShared
 * to provide shared sync functionality, for more info look at @see SdCardSyncServiceShared
 *
 */
public class SdSyncServerShared extends SdSyncServer {
	...
	/**
	 * {@inheritDoc}
	 * for shared, we also read all manifests from the shares
	 */
	@Override
	...
	/**
	 * serializes the notes. shared notes are encrypted with an appropriate crypto scheme
	 * which might be different from the normal sync one
	...
	/**
	 * creates the manifest file and also the shared manifests
	 * @param newandupdated
	 * @param newRev
	 * @param remainingIds
	 * @param oldRev
	...
	private boolean createManifest(List<Note> newandupdated, long newRev, List<String> remainingIds, long oldRev) {
		...
		try {
			String data = mw.serialize(newandupdatedNormal, this.serverId, newRev, remainingIdsNormal, oldRev, notesWithRevs);
