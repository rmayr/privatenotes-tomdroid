...
/**
 * Implements asymmetric cryptography (PGP compatible) on android using
 * the APG application. This means APG _MUST_ be installed for this to work.
 * 
 * This implementation is not threadsafe AT ALL! You can only execute one operation
 * after the other, because the APG utility will be invoked in between and we have to
 * capture its return values.
 * You have to call the appropriate get-method to optain the result before doing the
 * next operation, because after another operation you will get the results of that op.
 * 
 * Furthermore this class needs to get the Activity-Result calls. This means:
 * you must override the onActivityResult method in every activity that can be
 * displayed while this class is used, then foreward the results to the object that
 * made the call!
 *
 */
public class PgpCryptoScheme implements AsymmetricCryptoScheme {
	...
	/**
	 * gets the currently selected encryption keys
	 * @return
	 */
	...
	/**
	 * sets the keys that will be preselected for encryption 
	...
	/**
	 * resets the results from the last invocation of APG
	 */
	private void reset() {
	...
	/**
	 * sets the activity that is used to start the APG activity from
	 * @param activity
	 */
	...
	/**
	 * checks if the APG app is available
	 * @return true if avail
	 */
